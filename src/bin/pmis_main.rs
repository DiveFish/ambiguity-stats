...
use clap::{Arg, App};
use conllx::Token;
use ambiguity_stats::{read_sentences, get_all_files, get_ngrams, ngrams_to_file, sort_pmi_file, deprel_ngrams_to_file, get_deprel_ngrams};
...
fn collect_ngrams(files: Vec<String>, filename_template: &str, ngram_size: usize) {
    for file in &files {
        deprel_ngrams_to_file(filename_template,
