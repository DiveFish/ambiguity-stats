...
pub fn errors(golddata: &[Vec<Token>], parserdata: &[Vec<Token>]) {   //Check if &[[Token]] works
    ...
    // Dependency labels
    let labels = ["-PUNCT-", "-UNKNOWN-", "ADV", "APP", "ATTR", "AUX", "AVZ", "CJ", "DET",
        "EXPL", "GMOD", "gmod-app", "GRAD", "KOM", "KON", "KONJ", "koord", "NEB", "OBJA", "OBJC",
        "OBJD", "OBJG", "OBJI", "OBJP", "PAR", "PART", "PN", "PP", "PRED", "REL", "ROOT", "S",
        "SUBJ", "SUBJC", "ZEIT"];
    ...
    for label in labels.iter() {
        ...
        for (gold_sent, parser_sent) in golddata.iter().zip(parserdata.iter()) {
            ...
            let (attachments, combined_errors, head_errors, label_errors, wrong_labels) = get_errors_by_labels(&label, gold_sent, parser_sent);
        ...
        let las = (1.0 - (all_combined_errors + all_head_errors + all_label_errors) as f32 / all_attachments as f32) * 100.0;
        let uas = (1.0 - ((all_combined_errors + all_head_errors) as f32 / all_attachments as f32)) * 100.0;
        eprintln!("{:?}\n{:?}", las, uas);

        let error_sum = all_combined_errors + all_head_errors + all_label_errors;

        //print!("{:?}\n{:?}\n{:?}\n", all_combined_errors, all_head_errors, all_label_errors);
